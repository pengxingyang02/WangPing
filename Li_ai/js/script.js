...
camera.position.z = isMobile ? 2.3 : 1.8;


// 轨迹球控制器
const controls = new THREE.TrackballControls(camera, renderer.domElement);
controls.noPan = true;
controls.maxDistance = isMobile ? 4 : 3;
controls.minDistance = isMobile ? 1.2 : 0.7;
...
new THREE.OBJLoader().load(
  ...
  (obj) => {
    ...
    const heartScale = isMobile ? 0.06: 0.06;
    heart.geometry.scale(heartScale, heartScale, heartScale);
...
class SparkPoint {
  ...
  update(a) {
    ...
    const noise2 =
      ...
      1;
    this.one = this.pos.clone().multiplyScalar(1.02 + noise * 0.18 * beat.a);
    this.two = this.pos
      .clone()
...
function init(a) {
  positions = [];
  colors = [];
  const PARTICLE_COUNT = isMobile ? 12000 : 20000;
